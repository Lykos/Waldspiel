...
/**
 * this class represents the position of an army/a building and contains certain
 * vector geometric methods and some more specific stuff for the forest map.
 * 
 * @author bernhard
 */
public class Position implements Serializable {
	...
	private static Direction[] EAST_DIRECTIONS = { Direction.EAST,
			Direction.NORTH_EAST, Direction.SOUTH_EAST };
	private static Direction[] WEST_DIRECTIONS = { Direction.WEST,
			Direction.NORTH_WEST, Direction.SOUTH_WEST };
	private static Direction[] NORTH_DIRECTIONS = { Direction.NORTH,
			Direction.NORTH_WEST, Direction.NORTH_EAST };
	private static Direction[] SOUTH_DIRECTIONS = { Direction.SOUTH,
			Direction.SOUTH_EAST, Direction.SOUTH_WEST };
	private static Direction[] NON_HORIZONTAL_DIRECTIONS = { Direction.SOUTH,
			Direction.STAY, Direction.NORTH };

	private int directionDX(Direction direction) {
		// TODO: This is terrible!
		if (Arrays.asList(EAST_DIRECTIONS).contains(direction)) {
			return 1;
		} else if (Arrays.asList(WEST_DIRECTIONS).contains(direction)) {
			return -1;
		} else {
			return 0;
		}
	...
	private int directionDY(Direction direction) {
		// TODO: This is terrible!
		if (Arrays.asList(NORTH_DIRECTIONS).contains(direction)) {
			return 1;
		} else if (Arrays.asList(SOUTH_DIRECTIONS).contains(direction)) {
			return -1;
		} else {
			return 0;
		}
	 ...
	 */
	public boolean validDirection(Direction direction)
			throws InvalidDirectionException {
		return inBounds(x + directionDX(direction), y + directionDX(direction))
	 ...
	 *            where to go.
	 * @throws InvalidDirectionException
	 *             if this direction does not exist.
	 * @throws InvalidPositionException
	 *             if this would reach a position outside the map.
	 */
	public void goDirection(Direction direction)
			throws InvalidDirectionException, InvalidPositionException {
	...
	/**
	 * If an army moves, there are some squares which are not seen any more by
	 * this army afterwards. This method calculates those points. This is a very
	 * critical method which has to be improved and debugged.
	 * 
	 * @param direction
	 *            the direction the army goes to.
	 * @param sightRange
	 *            the sight range of the army
	 * @return the coordinates of that triangle in an array of width two, first
	 *         the x, then the y-coordinate.
	 * @throws InvalidDirectionException
	 *             if the direction is not valid.
	 */
	public int[][] getTriangle(Direction direction, int sightRange)
			throws InvalidDirectionException {
		// TODO improve this and test it.
		int[][] yRectangle, xRectangle;
		int dx = directionDX(direction) * -1;
		int dy = directionDY(direction) * -1;
	 ...
	 * after the movement with a given sight range.
	 * 
	 * @param direction
	 *            where to go
	 * @param sightRange
	 *            how far this army sees
	 ...
	 *             if the direction is not valid.
	 * @throws InvalidPositionException
	 *             if this would reach a point outside the map.
	 */
	public int[][] goDirection(Direction direction, int sightRange)
			throws InvalidDirectionException, InvalidPositionException {
	...
	/**
	 * calculates, into which direction one has to go from this point to reach
	 * another point.
	 * 
	 * @param destination
	 ...
	 */
	public Direction toDestination(Position destination) { // Bad code
		Direction[] candidates = new Direction[3];
		if (destination.getX() > x) {
			System.arraycopy(EAST_DIRECTIONS, 0, candidates, 0, 3);
		} else if (destination.getX() == x) {
			System.arraycopy(NON_HORIZONTAL_DIRECTIONS, 0, candidates, 0, 3);
		} else {
			System.arraycopy(WEST_DIRECTIONS, 0, candidates, 0, 3);
		}
	 ...
	 * @param y
	 *            y-coordinate of the other point.
	 * @return integer value of the distance.
	 */
	 ...
	 * 
	 * @param distance
	 *            maximum distance of the returned points
	 * @return an array of width two with all the coordinates of the square,
	 *         first the x and then the y coordinate.
	 */
	public int[][] getSurrounding(int distance) {
		int startX = Math.max(0, x - distance), endX = Math.min(x + distance,
				100);
		int startY = Math.max(0, y - distance), endY = Math.min(y + distance,
				100);
