...
/**
 * This is the object that stores which squares a side can see.
 * Squares can either be seen by buildings or by units.
 * @author bernhard
 *
 */
public class ForestMap extends Observable implements Serializable {
	...
	/**
	 * If an army does not see anything any more for any
	 * reason, this method should be called. It tests, if the
	 * squares, which were seen by the army before, are still
	 * seen.
	 ...
	 * @param position the position where the new building lies
	 * @param sightRange the sightrange of the new building
	 */
	protected void addBuilding(Position position, int sightRange) {
	...
	/**
	 * If a building does not see anything any more for any
	 * reason, this method should be called. It tests, if the
	 * squares, which were seen by the building before, are still
	 * seen.
	 * @param position the position of the removed building
	 * @param sightRange the sightRange of that building
	 */
	protected void removeBuilding(Position position, int sightRange) {
	...
	/**
	 * Tests, if the player sees a given position
	 * and if yes, returns some information about this position.
	 * The information contains only rough information like who has
	 * an army or a building on that square. (Not which building, 
	 * what kind of army etc.)
	 * @param x x-coordinate of the position to look at
	 * @param y y-coordinate of the position to look at
	 * @return an int with some information about a given square, 
	 * whose meaning can be looked up in class Tree.
	 */
	...
	/**
	 * Same as lookAt, but returns an int, so that computers
	 * or graphical views can use it more efficiently without
	 * having to get the information out of a String. 
	 * @param x x-coordinate of the position to look at
	 * @param y y-coordinate of the position to look at
	 * @return an int with some information about a given square, 
	 * whose meaning can be looked up in class Tree.
	...
	/**
	 * Does the same as lookAt(int x, int y), but takes
	...
	/**
	 * checks if it still sees a given set of points.
	 * This is for example used if an army moves and
	 * doesn't see some points any more.
	 * @param criticalPoints the points to be tested
	 */
	protected void checkArmySight(int[][] criticalPoints) {
